...
(function($){$.fn.bxCarousel=function(options){
	...
	var defaults={move:1,display_num:1,speed:500,margin:0,auto:true,auto_interval:3000,auto_dir:'next',auto_hover:true,next_text:'next',next_image:'',prev_text:'prev',prev_image:'',controls:true};
	...
	return this.each(function(){
		...
		if(options.controls){
			if(options.next_image!=''||options.prev_image!=''){var controls='';}
			else{var controls='';}
		}
$this.parent('.bx_container').wrap('<div class="bx_wrap"></div>').css({'float':'left','position':'relative','width':890,'overflow':'hidden','margin-left':60,'margin-right':0}).before(controls);
var w=li.slice(0,options.display_num).clone();var last_appended=(options.display_num+options.move)-1;$this.empty().append(w);get_p();get_a();$this.css({'position':'relative','left':-(seg)});

$this.parent().parent().siblings('.next').click(function(){
	slide_next();
	clearInterval(j);
	clicked=true;
...
$this.parent().parent().siblings('.prev').click(function(){
	slide_prev();
	clearInterval(j);
	clicked=true;
	return false;});
...
if(options.auto){
	...
	if(options.auto_hover&&clicked!=true){
		$this.find('li').live('mouseenter',function(){if(!clicked){clearInterval(j);}});
...
$(function(){
	var ss=	setInterval(function(){ $(".next").click()},5000);
	$("#example1 li").hover(function(){
		clearInterval(ss);
	
	}).mouseleave(function(){
		ss=	setInterval(function(){ $(".next").click()},5000);
