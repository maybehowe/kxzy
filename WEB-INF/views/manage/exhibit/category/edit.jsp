 ...
 $(function() {
	 ...
	 $("#cate_name")
	        ...
	        .inputValidator({
	        	min:1,
	        	max:20,
	        	onErrorMax:"展品类别长度不能超过20个字符",
		        onError:"展品类别不能为空,请确认"
	        	})
	        .regexValidator({
	        	regExp:"chineseenglish",
	        	dataType:"enum",
	        	onError:"展品类别名只能包含中文或英文"
	        	})
		    .ajaxValidator({
				dataType : "json",
				async : true,
				type : "post",
				url : "${contextPath}/manage/exhibit/category/validateExhibitCategoryName.rt",
				data:{"editExhibitCate":"${cate.cateName}","add_exhibitCate":false},
				success : function(result){
		            if(result) return true;
					return "该展品类别名已存在，请更换展品类别名";
				},
				buttons: $("#button"),
				error: function(jqXHR, textStatus, errorThrown){
					alert("服务器没有返回数据，可能服务器忙，请重试"+errorThrown);
					},
				onError : "该展品类别名已存在，请更换展品类别名"
			  });
	   $("#cate_description")
		       .formValidator({
		    	     empty:true,
...
<div class="wrapper">
	...
	<jsp:include page="/WEB-INF/views/manage/common/left.jsp" />
		<div class="right">
			<div class="main">
				<div class="tit">编辑展品类别</div>
				<div class="line">展品类别详情：</div>
				  <form id="cate_Form"  action="${contextPath}/manage/exhibit/category/submitEdit.rt" method="post">
