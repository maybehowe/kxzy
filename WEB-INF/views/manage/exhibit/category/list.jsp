...
$(function(){
	...
	$("#delSelected").click(function(){
		...
		if(arrayId.length > 0){
			...
			if(confirm("确定删除？")){
				var url = "${contextPath }/manage/exhibit/category/delete.rt";
				var params = {
					"ids" : ids,
				};
					...
					if (result.flag){
						 $.scojs_message('删除成功',{
							 ...
							 "onAfterHidden" : function(){
								 $("#searchForm").attr("action","${contextPath }/manage/exhibit/category/list.rt");
								 $("#searchForm").submit();
							 }
						 });
					} else {
...
//编辑
function edit(id){
	$("#searchForm").attr("action","${contextPath }/manage/exhibit/category/showEdit.rt?cateId="+id);
	$("#searchForm").submit();
}
//删除
function del(cateId){
...
//详情
function detail(id){
	$("#searchForm").attr("action","${contextPath }/manage/exhibit/category/showDetail.rt?cateId="+id);
	$("#searchForm").submit();
}
</script>
</head>
...
<div class="wrapper">
	...
	<div class="content">
		...
		<div class="right">
			<div class="buts">
				<a href="${contextPath}/manage/exhibit/category/showAdd.rt?viewId=${viewId}&leftId=${leftId}">添加科普展品类别</a>
			</div>
			<form name="searchForm" id="searchForm" action="${contextPath}/manage/exhibit/category/list.rt" method="post">
			...
			<div class="main">
				  ...
				  <c:forEach items="${page.results}" var="cate">
				  	 <tr>
						...
						<c:choose>
						   ...
						   <%--${rt:escapeHtml(fn:substring(cate.description,0,10))} ........
						       ${rt:escapeHtml(fn:substring(cate.description, fn:length(cate.description)-10,-1))}--%>
				...
				<div class="page">
					<div class="fl"><input type="button" id="delSelected" value="删除选中展品类别" class="sub2" style="*margin-left:-170px;" /></div>
					<jsp:include page="/WEB-INF/views/manage/common/page.jsp" >
					<jsp:param value="${contextPath}/manage/exhibit/category/list.rt" name="pageUrl"/>
