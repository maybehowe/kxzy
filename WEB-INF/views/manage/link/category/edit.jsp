 ...
 $(function() {
	 ...
	 $("#cate_name")
	        ...
	        .inputValidator({
	        	min:1,
	        	max:20,
	        	onErrorMax:"友情链接类别长度不能超过20个字符",
		        onError:"友情链接类别不能为空,请确认"
	        	})
	        .regexValidator({
	        	regExp:"chineseenglish",
	        	dataType:"enum",
	        	onError:"友情链接类别名只能包含中文或英文"
	        	})
		    .ajaxValidator({
				dataType : "json",
				async : true,
				type : "post",
				url : "${contextPath}/manage/link/category/validateLinkCategoryName.rt",
				data:{"editLinkCate":"${cate.cateName}","add_linkCate":false},
				success : function(result){
		            if(result) return true;
					return "该友情链接类别名已存在，请更换友情链接类别名";
				},
				buttons: $("#button"),
				error: function(jqXHR, textStatus, errorThrown){
					alert("服务器没有返回数据，可能服务器忙，请重试"+errorThrown);
					},
				onError : "该友情链接类别名已存在，请更换友情链接类别名"
			  });
	   $("#cate_description")
		       .formValidator({
		    	     empty:true,
...
<div class="wrapper">
	...
	<jsp:include page="/WEB-INF/views/manage/common/left.jsp" />
		<div class="right">
			<div class="main">
				<div class="tit">编辑友情链接类别</div>
				<div class="line">友情链接类别详情：</div>
				  <form id="cate_Form"  action="${contextPath}/manage/link/category/submitEdit.rt" method="post">
