...
$(function(){
	...
	$("#delSelected").click(function(){
		...
		if(arrayId.length > 0){
			...
			if(confirm("确定删除？")){
				var url = "${contextPath }/manage/link/category/delete.rt";
				var params = {
					"ids" : ids,
				};
					...
					if (result.flag){
						 $.scojs_message('删除成功',{
							 ...
							 "onAfterHidden" : function(){
								 $("#searchForm").attr("action","${contextPath }/manage/link/category/list.rt");
								 $("#searchForm").submit();
							 }
						 });
					} else {
...
//编辑
function edit(id){
	$("#searchForm").attr("action","${contextPath }/manage/link/category/showEdit.rt?cateId="+id);
	$("#searchForm").submit();
}
//删除
function del(cateId){
...
//详情
function detail(id){
	$("#searchForm").attr("action","${contextPath }/manage/link/category/showDetail.rt?cateId="+id);
	$("#searchForm").submit();
}
</script>
</head>
...
<div class="wrapper">
	...
	<div class="content">
		...
		<div class="right">
			<div class="buts">
				<a href="${contextPath}/manage/link/category/showAdd.rt?viewId=${viewId}&leftId=${leftId}">添加友情链接类别</a>
			</div>
			<form name="searchForm" id="searchForm" action="${contextPath}/manage/link/category/list.rt" method="post">
			...
			<div class="main">
				...
				<div class="page">
					<div class="fl"><input type="button" id="delSelected" value="删除选中友情链接类别" class="sub2" style="*margin-left:-170px;" /></div>
					<jsp:include page="/WEB-INF/views/manage/common/page.jsp" >
					<jsp:param value="${contextPath}/manage/link/category/list.rt" name="pageUrl"/>
