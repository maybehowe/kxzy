...
$(function(){
    ...
    $("#delSelected").click(function(){
    	...
    	if(arrayId.length > 0){
			...
			if(confirm("确定删除？")){
				var url = "${contextPath }/manage/picture/delete.rt";
				var params = {
					"ids" : ids,
				};
					...
					if (result.flag){
						 $.scojs_message('删除成功',{
							 ...
							 "onAfterHidden" : function(){
								 $("#searchForm").attr("action","${contextPath }/manage/picture/list.rt?picturesetId="+${pictureset.id});
								 $("#searchForm").submit();
							 }
						 });
					} else {
...
function showUploadDiv(id){
	var uploadUrl = "${contextPath}/manage/picture/showUpload.rt?picturesetId="+id;	
	$.webox({
		height:400,
		width:900,
		bgvisibel:true,
...
function showEdit(id){
	 $("#searchForm").attr("action","${contextPath}/manage/picture/showEdit.rt?picId="+id);
	 $("#searchForm").submit();
}

//设封面
function setCover(id){
	var url = "${contextPath}/manage/picture/setPictureSetCover.rt";
	var params = {
		"pictureId" : id,
	...
	$.post(url,params,function(result) {
			if(result.flag){
  				 $("#searchForm").attr("action","${contextPath}/manage/picture/list.rt");
				 $("#searchForm").submit();
			} else {
	...
	<div class="content">
		...
		<div class="right">
			<div class="buts">
				...
				<a href="${contextPath}${rt:escapeHtml(returnUrl)}">返 回</a>
			</div>
			<form name="searchForm" id="searchForm" action="${contextPath}/manage/picture/list.rt" method="post">
			...
			<div style="padding-left: 9px;" id="picture_div">
			   <ul>
				  <c:forEach items="${page.results}" var="pic">
					...
					<p>
					    <a href="${contextPath}/manage/picture/view.rt?picId=${pic.id}&viewId=${viewId}&leftId=${leftId}" class="but_fanhui fl">查看</a>
			   ...
			   <div class="page">
					...
					<jsp:param value="${contextPath}/manage/picture/list.rt" name="pageUrl"/>
