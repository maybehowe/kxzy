...
$(function(){
    ...
    $("#delSelected").click(function(){
    	...
    	if(arrayId.length > 0){
			...
			if(confirm("确定删除？")){
				var url = "${contextPath }/manage/education/picture/delete.rt";
				var params = {
					"ids" : ids,
				};
					...
					if (result.flag){
						 $.scojs_message('删除成功',{
							 ...
							 "onAfterHidden" : function(){
								 $("#searchForm").attr("action","${contextPath }/manage/education/picture/list.rt?picturesetId="+${pictureset.id});
								 $("#searchForm").submit();
							 }
						 });
					} else {
...
function showUploadDiv(id){
	var uploadUrl = "${contextPath}/manage/education/picture/showUpload.rt?picturesetId="+id;	
	$.webox({
		height:400,
		width:900,
		bgvisibel:true,
...
function showEdit(id){
	 $("#searchForm").attr("action","${contextPath}/manage/education/picture/showEdit.rt?picId="+id);
	 $("#searchForm").submit();
}
...
function setCover(id){
	...
	$.post(url,params,function(result) {
			if(result.flag){
  				 $("#searchForm").attr("action","${contextPath}/manage/education/picture/list.rt");
				 $("#searchForm").submit();
			} else {
...
<div class="wrapper">
	...
	<div class="content">
		...
		<div class="right">
			...
			<form name="searchForm" id="searchForm" action="${contextPath}/manage/education/picture/list.rt" method="post">
			...
			<div style="padding-left: 9px;" id="picture_div">
			   <ul>
				  <c:forEach items="${page.results}" var="pic">
					<li style="float: left; padding-left: 10px;padding-bottom: 10px">
						<img src="${contextPath}/manage/education/picture/thumbnail.rt?picId=${pic.id}&size=176" alt="${pic.id}" class="cannel_img">
						<c:if test="${pictureset.isCover == pic.id}">
						   <div style="" class="fengmian">封面</div>
						</c:if>
					<p>
					    <a href="${contextPath}/manage/education/picture/view.rt?picId=${pic.id}&viewId=${viewId}&leftId=${leftId}" class="but_fanhui fl">查看</a>
			   ...
			   <div class="page">
					...
					<jsp:param value="${contextPath}/manage/education/picture/list.rt" name="pageUrl"/>
