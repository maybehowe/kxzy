...
$(function(){
	...
	/*
	  var cropInited = false;
	if(document.getElementById("originalImage").complete){
		log(" originalImage completed when test first.");
		cropInited = true;
		tryInitCrop();
	} else {
		log("originalImage not completed when test first.");
		$("#originalImage").load(function(){
			log("originalImage loaded when test first.");
			cropInited = true;
			tryInitCrop();
		});
	}
	if(document.getElementById("originalImage").complete){
		log(" originalImage completed when test second.");
		if(!cropInited){
			tryInitCrop();
		}
	}*/
	...
	<div class="content">
		...
		<div class="right">
			...
			<div class="main">
				...
				<div class="line">裁剪--主页置顶：</div>
				   <div>
	    			<img id="originalImage" src="${contextPath}/manage/education/picture/image.rt?picId=${picture.id}" alt="..." style="max-width:1215px"> 
	    		  </div>
				  <div id="preview-pane" style="margin-top:3px;">
					<div class="preview-container">
						<img src="${contextPath}/manage/education/picture/image.rt?picId=${picture.id}" class="jcrop-preview" alt="Preview"/>
					</div>
				</div>
		    	<div style="margin-top: 10px;">
		    		<div>
		    			<form action="${contextPath}/manage/education/picture/submitCrop.rt" method="post">
							...
							<input name="picture.id" type="hidden" value="${picture.id}">
